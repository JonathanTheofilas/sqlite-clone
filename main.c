...
ExecuteResult execute_insert(Statement* statement, Table* table) {
  ...
  if (cursor->cell_num < num_cells) {
    uint32_t key_at_index = *leaf_node_key(leaf, cursor->cell_num);
...
Cursor* internal_node_find(Table* table, uint32_t page_num, uint32_t key) {
  ...
  /* Binary search to find child to search */
  uint32_t min_index = 0;
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
  ...
  *leaf_node_next_leaf(old_node) = new_page_num;
  ...
  Starting from the right, move each key to its correct position.
  */
...
void create_new_root(Table* table, uint32_t right_child_page_num) {
  ...
  Re-initialize root page to be new internal node with two children.
  */
